...
(defmethod std.collection:split (delimiter (collection list) &key key from-end (start 0) end count test test-not remove-empty)
  (split delimiter collection :key key :from-end from-end :start start :end end :count count :test test :test-not test-not :remove-empty remove-empty))
