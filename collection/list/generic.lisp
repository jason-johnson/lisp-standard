...
(defmethod std.collection:split (delimiter (collection list) &key key from-end (start 0) end count (equal 'eql) test test-not remove-empty)
  (split delimiter collection :key key :from-end from-end :start start :end end :count count :equal equal :test test :test-not test-not :remove-empty remove-empty))
...
(defmethod std.collection:split-if-not (predicate (collection list) &key key from-end (start 0) end count remove-empty)
  (split-if-not predicate collection :key key :from-end from-end :start start :end end :count count :remove-empty remove-empty))